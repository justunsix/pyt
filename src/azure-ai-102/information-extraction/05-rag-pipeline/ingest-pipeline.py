...
def ingest_file(
    file_path, cu_client, openai_client, search_client, embedding_deployment
):
    ...
    for j, chunk in enumerate(chunks):
        ...
        resp = openai_client.embeddings.create(input=chunk, model=embedding_deployment)
...
def run_ingestion(
    cu_client, openai_client, search_client, embedding_deployment, manifest
):
    ...
    log(
        f"Ingestion complete — {len(pending)} file(s), "
...
def main():
    parser = argparse.ArgumentParser(description="Automated RAG ingestion pipeline")
    parser.add_argument(
        "--watch",
        action="store_true",
        help=f"Continuously watch for new files (polls every {POLL_INTERVAL} seconds)",
    )
    parser.add_argument(
        "--reset",
        action="store_true",
    ...
    if (
        not foundry_endpoint
        or not foundry_key
        or not embedding_deployment
        or not search_endpoint
        or not search_key
    ):
        raise ValueError("Some environment variables in .env file not set.")
    ...
    if args.watch:
        log(f"Watching '{DATA_FOLDER}/' for new documents (press Ctrl+C to stop)...\n")
        try:
            while True:
                found = run_ingestion(
                    cu_client,
